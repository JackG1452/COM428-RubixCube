...
private:
	...
	//bool for wireframe if statement
	bool mIsWireframe = false;

	//bool for allowing cube to be rotated by holding R
	bool RotateCube = false;

	//int to differ between different numbers for culling modes
	int cullingMode = -1;

	//bool to update cube to reset
	bool resetCube = false;

	//Cameras eye position
	XMFLOAT3 mEyePos = { 0.0f, 0.0f, 0.0f };

	//View matrix for defining virtual camera in world
	XMFLOAT4X4 mView = MathHelper::Identity4x4();

	//Projection matrix for defining clipping planes, fov, aspect ratio etc...
	XMFLOAT4X4 mProj = MathHelper::Identity4x4();

	//Camera View for horizontal movement
	float mTheta = 1.3f*XM_PI;

	//Camera View for vertical movement
	float mPhi = 0.35f*XM_PI;

	//camera view radius for spherical 
	float mRadius = 12.0f;

	//float for rotation - updates value in updateObjectCBs
	float rotation = 0.0f;

	//float for rotation speed - multipled by delta time for normal speed across devices
	float rotateSpeed = 50.0f;

	//int to control which rotate axis value gets used
	int rotateAxis = 0;
...
{
	//If reset is true, clears current rendered items, calls build items function again(so will be in default positons again)
	//reset players view, objects Pso's state
	if (resetCube == true)
	{
		resetCube = false;
		mRitemLayer->clear();
...
void NormalMapApp::Draw(const GameTimer& gt)
{
	...
	//If statements to control which Pso's get called to visualy display the cube, as either being culled or wireframe
	//if culling is negative, can access wireframe/opaque calls
	if (cullingMode == -1)
	{
		//if true makes cubes wireframe
		if (mIsWireframe)
		{
			ThrowIfFailed(mCommandList->Reset(cmdListAlloc.Get(), mPSOs["opaque_wireframe"].Get()));
		}
		//else everything opaque
		else
		{
			ThrowIfFailed(mCommandList->Reset(cmdListAlloc.Get(), mPSOs["opaque"].Get()));
		}
	...
	{
		//else switch between different cull modes depedning on which key is selected to change value
		if (cullingMode == 0)
		{
...
//Move camera with mouse - holding left click to rotate around lookat position 
//holding right click to zoom further in and out from lookat
void NormalMapApp::OnMouseMove(WPARAM btnState, int x, int y)
{
	if ((btnState & MK_LBUTTON) != 0)
	{
		...
		//restricts so cant rotate fully round cube otherwise get a jump/rotate effect of camera switching to allow this
	...
	else if ((btnState & MK_RBUTTON) != 0)
	{
		...
		// Restrict the radius.
		//restrict how far in and out you can go from lookat position
...
//Different keyboard input commands
void NormalMapApp::OnKeyboardInput(const GameTimer& gt)
{
	//get delta time for equations to make it more compatiable over multiple devices
	const float dt = gt.DeltaTime();

	//Left key - to move camera left around cube, same as mouse camera movement
	...
	//Right key - to move camera right around cube, same as mouse camera movement
	...
	//Up key - to move camera up around cube, same as mouse camera movement
	if (GetAsyncKeyState(VK_UP) & 0x8000)
	{
		...
		// Restrict the angle mPhi.
		//same restriction as mouse camera movement
		mPhi = MathHelper::Clamp(mPhi, 0.1f, MathHelper::Pi - 0.1f);
	}

	//Down key - to move camera down around cube, same as mouse camera movement
	...
	//If R pressed enables rotate cube and Either pressing X, Y, Z aswell will rotate cube in that axis
	...
	//Sets value needed to differ between calls in updateObjectCBs
	//rotation value declared here so will work more efficently than in updateObjectCBs
	//otherwise constantly rotating without change
	//converted to correct value for rotation, uses dt * speed to make it faster and same across devices
...
void NormalMapApp::UpdateCamera(const GameTimer& gt)
{
...
//Constantly checking on each frame to see if any changes to the objects in constant buffer
void NormalMapApp::UpdateObjectCBs(const GameTimer& gt)
{
	auto currObjectCB = mCurrFrameResource->ObjectCB.get();
	for(auto& e : mAllRitems)
	{
		//If you want specific faces to be only rotated their values are declared below
		...
		//Different If's depending on which rotation axis choosen
		//Only works if R has been pressed to enable rotateCube to be true
		if (RotateCube == true)
		{
			//Rotation for X axis
			if (rotateAxis == 1)
...
void NormalMapApp::LoadTextures()
{
	...
	{
		L"Textures/Atlas.dds",
		L"Textures/Atlas_nMap.dds",
		L"Textures/tile.dds",
...
void NormalMapApp::BuildRenderItems()
{
	...
	//XMStoreFloat4x4(&boxRitem->World, XMMatrixTranslation(0.0f, 0.5f, 0.0f));
	...
	//loop to create 27 cubes, moving them over and up as needed, storing them in their own world matrix and the local world matrix
	//gets their geometry, materials, textures, etc
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
